...
def convert_phi2phik(basis, phi_val, phi_grid=None, size=1.0):
    ...
    if phi_grid is None:
        ...
        phi_grid = np.meshgrid(*[np.linspace(0.0, size, int(np.sqrt(len(phi_val))))
                                for _ in range(2)])
...
def convert_phik2phi(basis, phik, phi_grid=None, size=1.0):
    ...
    '''
    if phi_grid is None:
        print('--Assuming square grid (default resolution 50)')
        phi_grid = np.meshgrid(*[np.linspace(0.0, size)
                                for _ in range(2)])
        phi_grid = np.c_[phi_grid[0].ravel(), phi_grid[1].ravel()]
